...
const DEFAULT_CAPABILITIES: AppCapabilities = {
  ...
  supportsEditorRestart: false,
  supportsTrayUsageDisplay: true,
  supportsAutoStart: true,
  supportsOpencodeDesktopRestart: false,
  cloudSyncReason: "Cloud sync is not available in Windows v1.",
  editorRestartReason: "Automatic editor restart is disabled in Windows v1.",
  trayUsageDisplayReason: null,
  autoStartReason: null,
  opencodeDesktopRestartReason: "Opencode desktop restart is disabled in Windows v1.",
};
...
export function useCodexController() {
  ...
  const loadCapabilities = useCallback(async () => {
    try {
      const data = await invoke<AppCapabilities>("get_runtime_capabilities");
  ...
  const loadInstalledEditorApps = useCallback(async () => {
    if (!capabilities.supportsEditorRestart) {
  ...
  }, [capabilities.supportsEditorRestart]);

  const loadOpencodeDesktopAppInstalled = useCallback(async () => {
    if (!capabilities.supportsOpencodeDesktopRestart) {
  ...
  }, [capabilities.supportsOpencodeDesktopRestart]);
  ...
  const onSwitch = useCallback(
    async (account: AccountSummary) => {
      ...
      try {
        ...
        if (settings.restartEditorsOnSwitch && capabilities.supportsEditorRestart) {
