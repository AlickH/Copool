...
pub(crate) struct AppSettings {
    ...
    pub(crate) auto_smart_switch: bool,
...
pub(crate) struct AppSettingsPatch {
    ...
    pub(crate) auto_smart_switch: Option<bool>,
...
pub(crate) struct AppCapabilities {
    pub(crate) supports_cloud_sync: bool,
    pub(crate) supports_editor_restart: bool,
    pub(crate) supports_tray_usage_display: bool,
    pub(crate) supports_auto_start: bool,
    pub(crate) supports_opencode_desktop_restart: bool,
    pub(crate) cloud_sync_reason: Option<String>,
    pub(crate) editor_restart_reason: Option<String>,
    pub(crate) tray_usage_display_reason: Option<String>,
    pub(crate) auto_start_reason: Option<String>,
    pub(crate) opencode_desktop_restart_reason: Option<String>,
}

impl AppCapabilities {
    pub(crate) fn windows_v1() -> Self {
        Self {
            supports_cloud_sync: false,
            supports_editor_restart: false,
            supports_tray_usage_display: true,
            supports_auto_start: true,
            supports_opencode_desktop_restart: false,
            cloud_sync_reason: Some(
                "Cloud sync is not available in Windows v1. Use local storage for now."
                    .to_string(),
            ),
            editor_restart_reason: Some(
                "Automatic editor restart is disabled in Windows v1.".to_string(),
            ),
            tray_usage_display_reason: None,
            auto_start_reason: None,
            opencode_desktop_restart_reason: Some(
                "Opencode desktop restart is disabled in Windows v1.".to_string(),
            ),
        }
    }
}
...
impl StoredAccount {
    ...
    pub(crate) fn to_summary(
        &self,
        current_selection_account_id: Option<&str>,
        ...
        let is_current = current_variant_key
            ...
            .unwrap_or_else(|| {
                current_account_key
                    ...
                    .or_else(|| {
                        current_selection_account_id
                            .map(|account_id| account_id == self.account_id.as_str())
                    })
                    .unwrap_or(false)
            });
        ...
        AccountSummary {
            ...
            team_alias: self.team_alias.clone(),
...
fn merge_duplicate_account_variant(left: StoredAccount, right: StoredAccount) -> StoredAccount {
    ...
    if preferred.team_name.is_none() {
        preferred.team_name = alternate.team_name.clone();
    }
    if preferred.team_alias.is_none() {
        preferred.team_alias = alternate.team_alias.clone();
    }
