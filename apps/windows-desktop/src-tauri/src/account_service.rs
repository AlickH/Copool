...
pub(crate) async fn list_accounts_internal(
    ...
    let current_selection_account_id =
        store.current_selection.as_ref().map(|selection| selection.account_id.as_str());
...
pub(crate) async fn import_auth_json_accounts_internal(
    ...
    let (imported_count, updated_count) = {
        ...
        let current_selection_account_id = store
            .current_selection
            .as_ref()
        ...
        for prepared in prepared_imports {
            let (_, updated_existing) = upsert_prepared_import(
                &mut store,
                prepared,
                current_selection_account_id.as_deref(),
                current_account_key.as_deref(),
                current_variant_key.as_deref(),
            );
            if updated_existing {
...
pub(crate) async fn refresh_all_usage_internal(
    ...
    for target in refresh_targets {
        handles.push(tauri::async_runtime::spawn(async move {
            ...
            let (auth_plan_type, auth_email, auth_team_name) = match &extracted {
                Ok(auth) => (
                    auth.plan_type.clone(),
                    auth.email.clone(),
                    auth.team_name.clone(),
                ),
    ...
    let store = {
        ...
        for account in &mut latest_store.accounts {
            ...
            account.team_name = outcome.auth_team_name.clone().or(account.team_name.clone());
...
fn apply_prepared_import_to_account(
    ...
    existing.team_name = team_name.or(existing.team_name.clone());
...
mod tests {
    ...
    fn prepared_import(
        ...
        PreparedImport {
            ...
            plan_type: Some(plan_type.to_string()),
            team_name: Some("workspace".to_string()),
    ...
    fn upsert_prepared_import_reuses_unknown_variant_placeholder() {
        ...
        let (summary, updated_existing) =
            upsert_prepared_import(&mut store, prepared, None, None, None);
    ...
    fn upsert_prepared_import_keeps_same_workspace_different_users_separate() {
        ...
        let (_, updated_first) = upsert_prepared_import(&mut store, first, None, None, None);
        let (_, updated_second) = upsert_prepared_import(&mut store, second, None, None, None);
